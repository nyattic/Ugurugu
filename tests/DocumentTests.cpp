...
Document documentWithStrokeCount(int count)
{
    ...
    for (int index = 0; index < count; ++index) {
        ...
        stroke.points = {
            {QPointF(
                 1.0 + static_cast<qreal>(index % 62),
...
private slots:
    ...
    void discardsFailedMacroWithoutUndoingEarlierHistory()
    {
        ...
        controller.undoStack()->undo();
        QCOMPARE(
            controller.document().wobbleAmount,
        ...
        QObject::connect(
            ...
            [&observedLastPixel](const QUuid &, const QImage &mask) {
                observedLastPixel = mask.isNull()
                    ? -1
                    : mask.constScanLine(0)[editCount - 1];
            });
    ...
    {
        constexpr int strokeLimit =
            DocumentLimits::maximumTotalStrokes;
        constexpr qint64 maximumAggregateLatencyMs = 20000;

        DocumentController controller;
        controller.loadDocument(documentWithStrokeCount(strokeLimit));
        const QUuid layerId = controller.document().activeLayerId;
        QCOMPARE(
            controller.document().layer(layerId)->strokes.size(),
            strokeLimit);
        ...
        QCOMPARE(
            renameStorage.retainedPreparedDocuments,
            qsizetype(0));
        QVERIFY2(
            renameMs < maximumAggregateLatencyMs,
            qPrintable(QStringLiteral(
                "64 large-document renames took %1 ms")
                           .arg(renameMs)));
        ...
        QCOMPARE(
            addStorage.retainedPreparedDocuments,
            qsizetype(0));
        QCOMPARE(
            controller.document().layer(addLayerId)->strokes.size(),
            strokeLimit);
        QVERIFY2(
            addMs < maximumAggregateLatencyMs,
            qPrintable(QStringLiteral(
                "64 large-document stroke additions took %1 ms")
                           .arg(addMs)));
        ...
        timer.restart();
        for (int index = 0; index < 64; ++index) {
            controller.setWobbleAmount(
                2.0 + static_cast<qreal>(index) / 100.0);
        }
        ...
        QCOMPARE(
            scalarStorage.retainedPreparedDocuments,
            qsizetype(0));
        controller.undoStack()->undo();
        ...
        QVERIFY2(
            scalarMs < maximumAggregateLatencyMs,
            qPrintable(QStringLiteral(
                "64 large-document scalar edits took %1 ms")
                           .arg(scalarMs)));

        controller.loadDocument(controller.document());
        const QUuid clearLayerId =
            controller.document().activeLayerId;
        timer.restart();
        controller.clearLayer(clearLayerId);
        const qint64 clearMs = timer.elapsed();
        QCOMPARE(
            controller.undoStack()->storageStats().retainedStrokes,
            qsizetype(strokeLimit));
        QCOMPARE(
            controller.document().layer(clearLayerId)->strokes.size(),
            0);
        timer.restart();
        controller.undoStack()->undo();
        ...
        QCOMPARE(
            controller.document().layer(clearLayerId)->strokes.size(),
            strokeLimit);
        QVERIFY(clearMs < maximumAggregateLatencyMs);
        QVERIFY(restoreClearMs < maximumAggregateLatencyMs);

        controller.loadDocument(controller.document());
        const QUuid removedLayerId =
            controller.document().activeLayerId;
        timer.restart();
        ...
        QCOMPARE(
            controller.document().layer(removedLayerId)->strokes.size(),
            strokeLimit);
        QVERIFY(removeMs < maximumAggregateLatencyMs);
        QVERIFY(restoreRemoveMs < maximumAggregateLatencyMs);

        qInfo().nospace()
            << "large history latency ms: rename=" << renameMs
            << ", add=" << addMs
            << ", scalar=" << scalarMs
            << ", clear=" << clearMs
    ...
    void preflightsWholeMacroBeforeMovingHistory()
    {
        ...
        DocumentControllerTestAccess::failHistoryPrepareAfter(
            controller,
            0);
        controller.undoStack()->undo();
    ...
    void blocksHistoryMutationDuringSynchronousCallbacks()
    {
                ...
                if (phase == 1) {
                    ...
                    pushStateWasAtomic =
                        ...
                        && DocumentControllerTestAccess::contentRevision(
                               controller)
                            == 1
                        && DocumentControllerTestAccess::historyNode(
                               controller)
                            != 0;
                    controller.undoStack()->undo();
                    controller.undoStack()->redo();
                    controller.renameLayer(
                        firstLayerId,
                        QStringLiteral("Rejected nested rename"));
                    QImage selected(1, 1, QImage::Format_Grayscale8);
                    selected.fill(255);
                    controller.pushSelectionStateCommand(
        ...
        QCOMPARE(
            controller.document().layer(firstLayerId)->name,
            QStringLiteral("Layer 1"));
        QCOMPARE(
            controller.document().layer(firstLayerId)->strokes.size(),
            1);
        ...
        QCOMPARE(controller.undoStack()->index(), 0);
        QVERIFY(
            controller.document().layer(firstLayerId)->strokes.isEmpty());
        QCOMPARE(
            controller.document().layer(firstLayerId)->name,
            QStringLiteral("Layer 1"));
        QCOMPARE(controller.document().activeLayerId, secondLayerId);
    ...
    void commitsAccumulatedSelectionTransformAsOneCommand()
    {
        ...
        stroke.width = 8.0;
        stroke.points = {
            {QPointF(70.0, 70.0), 0.5},
            {QPointF(110.0, 80.0), 1.0}
        };
        ...
        QVERIFY(controller.transformSelection(
            layerId,
            {strokeId},
            accumulated));
        QCOMPARE(controller.undoStack()->count(), 1);
        ...
        QVERIFY(invertible);
        QCOMPARE(
            qvariant_cast<QTransform>(transformedSpy[1][2]),
            inverse);
    ...
    void roundTripsLayerAndStrokeSequenceDeltas()
    {
        ...
        QCOMPARE(
            layerOrder(controller.document()),
        ...
        for (int index = 0; index < 5; ++index) {
            ...
            item.points = {
                {QPointF(10.0 + index * 10.0, 40.0), 1.0}
            };
        ...
        const auto strokeOrder = [strokeLayerId](
                                     const Document &document) {
            QVector<QUuid> ids;
            for (const Stroke &item :
                 document.layer(strokeLayerId)->strokes) {
                ids.append(item.id);
            }
            return ids;
        };
        ...
        controller.removeStrokes(
            strokeLayerId,
            {originalIds[1], originalIds[3]});
        QCOMPARE(
            strokeOrder(controller.document()),
            QVector<QUuid>(
                {originalIds[0], originalIds[2], originalIds[4]}));
        controller.undoStack()->undo();
    ...
    void preservesProceduralFillAcrossSchemaFourMigration()
    {
        ...
        QString error;
        const std::optional<Document> migrated =
            DocumentSerializer::fromJson(
                QJsonDocument(schemaFour)
                    .toJson(QJsonDocument::Compact),
                &error);
        QVERIFY2(migrated.has_value(), qPrintable(error));
        QVERIFY(
            migrated->layers.first().strokes.last().fillMask.isNull());

        const QByteArray currentJson =
            DocumentSerializer::toJson(*migrated);
        QVERIFY(!currentJson.isEmpty());
        const QJsonObject currentRoot =
            QJsonDocument::fromJson(currentJson).object();
        QCOMPARE(
            currentRoot.value(QStringLiteral("schemaVersion")).toInt(),
            6);
        QCOMPARE(
            currentRoot.value(QStringLiteral("algorithmVersion")).toInt(),
            2);
        ...
        QVERIFY(
            reloaded->layers.first().strokes.last().fillMask.isNull());
        for (int frame = 0; frame < source.animationFrames; ++frame) {
            QCOMPARE(
                RenderEngine::render(*migrated, frame),
        ...
        const QByteArray changedJson =
            DocumentSerializer::toJson(changed);
        ...
        for (int frame = 0; frame < changed.animationFrames; ++frame) {
            const QImage changedFrame =
                RenderEngine::render(changed, frame);
            QCOMPARE(
                changedFrame,
                RenderEngine::render(*changedReloaded, frame));
            if (changedFrame
                != RenderEngine::render(
                    *reloaded,
                    frame % reloaded->animationFrames)) {
    ...
    void preservesFrozenSchemaFiveFillAcrossSchemaSixRoundTrip()
    {
        ...
        const std::optional<Document> migrated =
            DocumentSerializer::fromJson(
                QJsonDocument(schemaFive)
                    .toJson(QJsonDocument::Compact),
                &error);
        QVERIFY2(migrated.has_value(), qPrintable(error));
        QCOMPARE(
            migrated->layers.first().strokes.first().fillMask,
            fill.fillMask);

        const QByteArray schemaSixJson =
            DocumentSerializer::toJson(*migrated);
        ...
        QCOMPARE(
            schemaSixRoot.value(
                QStringLiteral("algorithmVersion")).toInt(),
            2);
        const QJsonObject schemaSixStroke =
            schemaSixRoot.value(QStringLiteral("layers"))
                ...
                .first()
                .toObject();
        QVERIFY(schemaSixStroke.contains(
            QStringLiteral("fillMaskId")));
        const std::optional<Document> reloaded =
            DocumentSerializer::fromJson(schemaSixJson, &error);
        QVERIFY2(reloaded.has_value(), qPrintable(error));
        QCOMPARE(
            reloaded->layers.first().strokes.first().fillMask,
            fill.fillMask);
        for (int frame = 0; frame < source.animationFrames; ++frame) {
            QCOMPARE(
                RenderEngine::render(*migrated, frame),
    ...
    void roundTripsOrderedFramebufferOperations()
    {
        ...
        stroke.width = 5.0;
        stroke.points = {
            {QPointF(8.0, 18.0), 1.0},
            {QPointF(28.0, 18.0), 1.0}
        };
        const QUuid strokeId = stroke.id;
        const QUuid layerId = document.activeLayerId;
        ...
        for (int y = 14; y < 27; ++y) {
            std::fill(
                selection.scanLine(y) + 10,
                selection.scanLine(y) + 37,
                255);
        }
        ...
        QVERIFY(
            loadedOperations[1].reframeOp
            == operations[1].reframeOp);
        QVERIFY(
            loadedOperations[2].pixelSelectionOp
            == operations[2].pixelSelectionOp);
        for (int frame = 0;
             frame < controller.document().animationFrames;
             ++frame) {
            QCOMPARE(
                RenderEngine::render(controller.document(), frame),
    ...
    void rejectsUnsafeSelectionTransformsAtomically()
    {
        ...
        for (int y = 8; y < 24; ++y) {
            std::fill(
                selection.scanLine(y) + 8,
                selection.scanLine(y) + 24,
                255);
        }

        QTransform huge;
        huge.scale(1.0e300, 1.0e300);
        QVERIFY(!makePixelSelectionOp(
            selection,
            huge,
            true,
            true));
        ...
        stroke.width = 4.0;
        stroke.points = {
            {QPointF(6.0, 16.0), 1.0},
            {QPointF(26.0, 16.0), 1.0}
        };
        const QUuid strokeId = stroke.id;
        const QUuid layerId = document.activeLayerId;
        document.layers.first().strokes = {stroke};
        ...
        operationDocument.layers.first().strokes.append(
            operationStroke);
        QJsonObject unsafeRoot =
            QJsonDocument::fromJson(
                DocumentSerializer::toJson(
                    operationDocument)).object();
        QJsonArray layers =
            unsafeRoot.value(QStringLiteral("layers")).toArray();
        QJsonObject layer = layers.first().toObject();
        QJsonArray strokes =
            layer.value(QStringLiteral("strokes")).toArray();
        QJsonObject serializedOperation =
            strokes.last().toObject();
        QJsonObject payload =
            serializedOperation.value(
                QStringLiteral("pixelSelection")).toObject();
        payload.insert(
            ...
            QJsonArray{
                ...
                0.0, 1.0e300, 0.0,
                0.0, 0.0, 1.0});
        serializedOperation.insert(
            QStringLiteral("pixelSelection"),
            payload);
        strokes[strokes.size() - 1] =
            serializedOperation;
    ...
    void clearsResizedLayerWithCurrentFramebufferEpoch()
    {
        ...
        stroke.width = 4.0;
        stroke.points = {
            {QPointF(4.0, 12.0), 1.0},
            {QPointF(24.0, 12.0), 1.0}
        };
        const QUuid layerId = document.activeLayerId;
        document.layers.first().strokes = {stroke};
        DocumentController controller;
        ...
        newStroke.points = {
            {QPointF(8.0, 8.0), 1.0},
            {QPointF(30.0, 20.0), 1.0}
        };
        loadedController.addStroke(layerId, newStroke);
        QVERIFY(!RenderEngine::render(
            loadedController.document(),
            0).isNull());
        QVERIFY(!DocumentSerializer::toJson(
            loadedController.document()).isEmpty());
    ...
    void resizesTransparentLayerWithoutFramebufferHistory()
    {
        ...
        erase.mode = StrokeMode::Erase;
        erase.points = {
            {QPointF(8.0, 20.0), 1.0},
            {QPointF(50.0, 20.0), 1.0}
        };
    ...
    void rejectsOverflowingSchemaSixBounds()
    {
        ...
        Stroke stroke;
        stroke.points = {
            {QPointF(4.0, 16.0), 1.0},
            {QPointF(28.0, 16.0), 1.0}
        };
        ...
        QJsonArray binaryMasks =
            badBinaryRoot.value(
                QStringLiteral("binaryMasks")).toArray();
        QJsonObject binaryMask =
            binaryMasks.first().toObject();
        binaryMask.insert(
        ...
        badBinaryRoot.insert(
            QStringLiteral("binaryMasks"),
            binaryMasks);
        QString error;
        QVERIFY(!DocumentSerializer::fromJson(
            QJsonDocument(badBinaryRoot)
                .toJson(QJsonDocument::Compact),
            &error));
    ...
    void resizesImageUndoably()
    {
        ...
        const Stroke &resizeOperation =
            resizedLayer.strokes.last();
        QCOMPARE(resizeOperation.mode, StrokeMode::Reframe);
        QVERIFY(resizeOperation.reframeOp.has_value());
        QCOMPARE(
            resizeOperation.reframeOp->mode,
            ReframeMode::Image);
        QCOMPARE(
            resizeOperation.reframeOp->sourceSize,
            QSize(100, 50));
        QCOMPARE(
            resizeOperation.reframeOp->targetSize,
            QSize(200, 100));
        QCOMPARE(
            resizeOperation.reframeOp->sampling,
            SamplingMode::Smooth);
    ...
    void resizesCanvasByTranslatingContentUndoably()
    {
        ...
        first.width = 10.0;
        first.points = {
            {QPointF(10.0, 20.0), 0.5},
            {QPointF(90.0, 60.0), 1.0}
        };
        Stroke second;
        second.width = 4.0;
        second.points = {
            {QPointF(25.0, 35.0), 1.0},
            {QPointF(50.0, 45.0), 1.0}
        };
        document.layers.first().strokes = {first, second};

        DocumentController controller;
        controller.loadDocument(document);
        QSignalSpy resizeSpy(
            &controller,
        ...
        QCOMPARE(
            resized.last().reframeOp->mode,
            ReframeMode::Canvas);
        QCOMPARE(
            resized.last().reframeOp->sourceSize,
            document.size);
        QCOMPARE(
            resized.last().reframeOp->targetSize,
            targetSize);
        QCOMPARE(
            resized.last().reframeOp->contentOffset,
            offset);

        QCOMPARE(resizeSpy.size(), 1);
        QCOMPARE(resizeSpy[0][0].toSize(), document.size);
        QCOMPARE(resizeSpy[0][1].toSize(), targetSize);
        const QTransform forward =
            qvariant_cast<QTransform>(resizeSpy[0][2]);
        ...
        const QTransform inverse =
            qvariant_cast<QTransform>(resizeSpy[1][2]);
    ...
    void supportsSameSizeCanvasOffsetsAndOffCanvasRoundTrip()
    {
        ...
        stroke.width = 7.0;
        stroke.points = {
            {QPointF(5.0, 20.0), 1.0},
            {QPointF(40.0, 30.0), 1.0}
        };
        document.layers.first().strokes.append(stroke);
        ...
        QVERIFY(controller.resizeCanvas(
            document.size,
            QPoint(-20, 5)));
        const Layer &translatedLayer =
            controller.document().layers.first();
        QCOMPARE(translatedLayer.strokes.size(), 2);
        QCOMPARE(
            translatedLayer.strokes.first().points,
            stroke.points);
        QCOMPARE(
            translatedLayer.strokes.first().width,
            stroke.width);
        QVERIFY(translatedLayer.strokes.first().clipMask.isNull());
        const Stroke &translated =
            translatedLayer.strokes.last();
        QCOMPARE(translated.mode, StrokeMode::Reframe);
        QVERIFY(translated.reframeOp.has_value());
        QCOMPARE(
            translated.reframeOp->contentOffset,
            QPoint(-20, 5));
        ...
        QVERIFY2(loaded.has_value(), qPrintable(error));
        QCOMPARE(
            loaded->layers.first().strokes.first().points,
            stroke.points);
        QCOMPARE(
            loaded->layers.first().strokes.last().reframeOp,
        ...
        QCOMPARE(
            loadedFile->layers.first().strokes.first().points,
            stroke.points);
    }
    ...
    void imageResizePreservesHiddenOffCanvasGeometry()
    {
        ...
        QCOMPARE(
            scaledLayer.strokes[1].reframeOp->contentOffset,
            QPoint(-20, 0));
        QCOMPARE(
            scaledLayer.strokes[2].reframeOp->mode,
            ReframeMode::Image);
    }
    ...
    void canvasCropMatchesTranslatedPaintRendering()
    {
        ...
        stroke.points = {
            {QPointF(5.0, 30.0), 1.0},
            {QPointF(55.0, 30.0), 1.0}
        };
        document.layers.first().strokes.append(stroke);
        ...
        QVERIFY(controller.resizeCanvas(
            QSize(40, 40),
            QPoint(-12, -8)));
        const Layer &croppedLayer =
            controller.document().layers.first();
        QCOMPARE(croppedLayer.strokes.size(), 2);
        QCOMPARE(
            croppedLayer.strokes.first().points,
            stroke.points);
        QCOMPARE(
            croppedLayer.strokes.first().width,
            stroke.width);
        QVERIFY(croppedLayer.strokes.last().reframeOp.has_value());
        QCOMPARE(
            croppedLayer.strokes.last().reframeOp->contentOffset,
            QPoint(-12, -8));
    ...
    void validatesStoredOffCanvasCoordinatesBySchema()
    {
        ...
        const std::optional<Document> legacy =
            DocumentSerializer::fromJson(
        ...
        const QByteArray unsafeJson =
            DocumentSerializer::toJson(unsafe);
        ...
        layers[0] = layer;
        unsafeRoot.insert(QStringLiteral("layers"), layers);
        const std::optional<Document> rejected =
            DocumentSerializer::fromJson(
                QJsonDocument(unsafeRoot)
                    .toJson(QJsonDocument::Compact),
                &error);
    ...
    void rejectsCanvasResizeThatWouldExceedCoordinateLimitAtomically()
    {
        ...
        QVERIFY(!controller.resizeCanvas(
            ...
            QPoint(
                static_cast<int>(
                    DocumentLimits::
                        maximumStoredCoordinateMagnitude)
                    + 1,
                0)));
        QCOMPARE(controller.undoStack()->count(), undoCount);
        QCOMPARE(controller.document().size, document.size);
    ...
    void movesOnlyTheSelectedPartOfAnIntersectingStroke()
    {
        ...
        QVERIFY(operation.pixelSelectionOp->drawDestination);
        QCOMPARE(
            operation.pixelSelectionOp->transform.map(
                QPointF(50.0, 50.0)),
            QPointF(50.0, 70.0));
        const QImage after =
            RenderEngine::render(controller.document(), 0);
        ...
        QCOMPARE(
            qvariant_cast<QImage>(transitionSpy[0][3]),
            selection);
        const QImage expectedNextMask =
            transformedSelectionSupport(
                ...
                operation.pixelSelectionOp->sampling);
        QCOMPARE(
            qvariant_cast<QImage>(transitionSpy[0][4]),
            expectedNextMask);
        ...
        QVERIFY(
            packedSource->packedMask.size()
                + packedNext->packedMask.size()
            < selection.sizeInBytes()
                + expectedNextMask.sizeInBytes());
        ...
        QCOMPARE(
            qvariant_cast<QImage>(transitionSpy[1][3]),
            expectedNextMask);
        QCOMPARE(
            qvariant_cast<QImage>(transitionSpy[1][4]),
            selection);
        controller.undoStack()->redo();
        QCOMPARE(RenderEngine::render(controller.document(), 0), after);
    ...
    void duplicatesOnlyTheSelectedPartOfAnIntersectingStroke()
    {
        ...
        QCOMPARE(
            copyOperation.pixelSelectionOp->transform.map(
                QPointF(50.0, 50.0)),
            QPointF(50.0, 70.0));
    ...
    void flipsOnlySelectedContentHorizontallyAndVertically()
    {
        ...
        horizontalStroke.points = {
            {QPointF(10.0, 45.0), 1.0},
            {QPointF(90.0, 55.0), 1.0}
        };
        const QUuid horizontalId = horizontalStroke.id;
        const QUuid horizontalLayer =
            horizontalDocument.activeLayerId;
        horizontalDocument.layers.first().strokes.append(
            horizontalStroke);
        QImage horizontalSelection(
            horizontalDocument.size,
            QImage::Format_Grayscale8);
        horizontalSelection.fill(0);
        for (int y = 35; y < 65; ++y) {
            std::fill(
                horizontalSelection.scanLine(y) + 20,
        ...
        QVERIFY(horizontalController.flipStrokes(
            horizontalLayer,
            {horizontalId},
            QPointF(50.0, 50.0),
            true,
        ...
        QCOMPARE(
            horizontallyFlipped.strokes.first().points,
            horizontalStroke.points);
        const Stroke &horizontalOperation =
            horizontallyFlipped.strokes.last();
        QCOMPARE(
            horizontalOperation.mode,
            StrokeMode::PixelSelection);
        ...
        QCOMPARE(
            horizontalOperation.pixelSelectionOp->transform.map(
                QPointF(30.0, 50.0)),
            QPointF(70.0, 50.0));
        const QImage horizontalRendered =
            RenderEngine::render(
                horizontalController.document(),
                0);
        ...
        verticalStroke.points = {
            {QPointF(45.0, 10.0), 1.0},
            {QPointF(55.0, 90.0), 1.0}
        };
        ...
        QImage verticalSelection(
            verticalDocument.size,
            QImage::Format_Grayscale8);
        verticalSelection.fill(0);
        for (int y = 20; y < 41; ++y) {
            std::fill(
                verticalSelection.scanLine(y) + 35,
                verticalSelection.scanLine(y) + 65,
                255);
        }

        const QImage verticalBefore =
            RenderEngine::render(verticalDocument, 0);
        DocumentController verticalController;
        verticalController.loadDocument(verticalDocument);
        QVERIFY(verticalController.flipStrokes(
            verticalLayer,
            {verticalId},
            QPointF(50.0, 50.0),
            false,
        ...
        QCOMPARE(
            verticallyFlipped.strokes.first().points,
            verticalStroke.points);
        const Stroke &verticalOperation =
            verticallyFlipped.strokes.last();
        QCOMPARE(
            verticalOperation.mode,
            StrokeMode::PixelSelection);
        ...
        QCOMPARE(
            verticalOperation.pixelSelectionOp->transform.map(
                QPointF(50.0, 30.0)),
            QPointF(50.0, 70.0));
    ...
    {
        Document source =
            Document::createDefault(QSize(65, 17));
        source.layers.first().name =
            QStringLiteral("레이어 \"A\"\n日本語");
        ...
        for (int y = 2; y < 15; ++y) {
            std::fill_n(
                clipMask.scanLine(y) + 4,
                41,
                static_cast<uchar>(255));
        }
        Stroke paint;
        paint.points = {
            {QPointF(4.25, 3.5), 0.5},
            {QPointF(44.75, 13.25), 1.0}
        };
        ...
        const std::optional<PixelSelectionOp> operation =
            makePixelSelectionOp(
                selection,
                QTransform::fromTranslate(1.5, -0.5),
                true,
                true);
        ...
        QCOMPARE(
            ...
            static_cast<qint64>(preparedJson.size()));
        QCOMPARE(
            DocumentSerializer::toJson(source),
            preparedJson);
    ...
    {
        Document source =
            Document::createDefault(QSize(8, 8));
        source.layers.first().name =
        ...
        source.layers.first().strokes = {
            paint,
            selection
        };

        QChar *rawName =
            source.layers.first().name.data();
        Stroke *rawStrokes =
            source.layers.first().strokes.data();
        StrokePoint *rawPoints =
            rawStrokes[0].points.data();
        uchar *rawImage =
            rawStrokes[0].clipMask.bits();
        char *rawPacked =
            rawStrokes[1].pixelSelectionOp
                ->packedMask.data();
        ...
        const Layer &frozenLayer =
            prepared->document().layers.first();
        QCOMPARE(
            frozenLayer.name,
            QStringLiteral("Layer"));
        QCOMPARE(frozenLayer.strokes[0].width, 5.0);
        QCOMPARE(
            frozenLayer.strokes[0].points[0].pressure,
            0.5);
        QCOMPARE(
            frozenLayer.strokes[0]
                .clipMask.constScanLine(0)[0],
            static_cast<uchar>(0xff));
        QCOMPARE(
            frozenLayer.strokes[1]
                .pixelSelectionOp->packedMask[0],
    ...
    {
        Document source =
            Document::createDefault(QSize(48, 32));
        Stroke firstStroke;
        firstStroke.points = {
            {QPointF(4.0, 5.0), 1.0}
        };
        ...
        changed.layers.first().name =
            QStringLiteral("이름 \"변경\"\n日本語");
        ...
        const QString expectedName =
            changed.layers.first().name;
        QChar *rawChangedName =
            changed.layers.first().name.data();
        ...
        QCOMPARE(
            prepared->document().layers.first().id,
            secondLayer.id);
        QCOMPARE(
            prepared->document().layers.first().name,
            expectedName);
        QCOMPARE(
            prepared->document().activeLayerId,
            secondLayer.id);
        for (const Layer &baseLayer :
             base->document().layers) {
            const Layer *preparedLayer =
                prepared->document().layer(baseLayer.id);
            QVERIFY(preparedLayer);
            QVERIFY(
                preparedLayer->strokes.constData()
    ...
    {
        Document source =
            Document::createDefault(QSize(32, 32));
        Stroke stroke;
        stroke.width = 5.0;
        stroke.points = {
            {QPointF(8.0, 9.0), 0.5}
        ...
        const Stroke *baseStrokes =
            base->document().layers.first()
                .strokes.constData();

        Document changed = base->document();
        Stroke *writableStrokes =
            changed.layers.first().strokes.data();
    ...
    {
        Document source =
            Document::createDefault(QSize(257, 129));
        QImage mask(
            source.size,
            QImage::Format_Grayscale8);
        ...
        const QByteArray firstJson =
            DocumentSerializer::toJson(*first, cache);
        QVERIFY(!firstJson.isEmpty());

        Document appended = first->document();
        Stroke plain;
        plain.points = {
            {QPointF(10.0, 20.0), 1.0},
            {QPointF(40.0, 60.0), 0.75}
        };
        ...
        Stroke duplicate =
            duplicated.layers.first().strokes.first();
        duplicate.id = QUuid::createUuid();
        duplicated.layers.first().strokes.append(
            std::move(duplicate));
        cache.resetStats();
        ...
        const qint64 originalKey =
            equalContent.layers.first()
                .strokes.first().clipMask.cacheKey();
        equalContent.layers.first()
            .strokes.first().clipMask =
                equalContent.layers.first()
                    .strokes.first().clipMask.copy();
        QVERIFY(
            equalContent.layers.first()
                .strokes.first().clipMask.cacheKey()
            != originalKey);
        cache.resetStats();
        const auto equalPrepared =
            DocumentSerializer::prepare(
                equalContent,
                cache,
                &*first,
        ...
        QImage &changedMask =
            changed.layers.first()
                .strokes.first().clipMask;
        const uchar previous = changedMask.constScanLine(0)[0];
        changedMask.scanLine(0)[0] =
            static_cast<uchar>(previous ^ 0xffU);
        cache.resetStats();
        const auto changedPrepared =
            DocumentSerializer::prepare(
                changed,
                cache,
                &*first,
    ...
    {
        Document source =
            Document::createDefault(QSize(16, 16));
        ...
        secondOperation.packedMask =
            firstOperation.packedMask;
        QVERIFY(
            firstOperation.packedMask.constData()
        ...
        const QJsonArray masks =
            QJsonDocument::fromJson(originalJson)
                .object()
        ...
        QByteArray &changedBytes =
            changed.layers.first().strokes.first()
                .pixelSelectionOp->packedMask;
        ...
        QCOMPARE(
            tinyCache.stats().binaryMaskCompressions,
            2ULL);
        QCOMPARE(tinyCache.payloadBytes(), 0);

        const qint64 onePayloadBytes =
            qCompress(packed, 6).size();
        DocumentSerializer::SerializationCache lruCache(
            onePayloadBytes);
        const auto lruPrepared =
            DocumentSerializer::prepare(source, lruCache);
        QVERIFY(lruPrepared.has_value());
        QVERIFY(lruCache.payloadBytes() > 0);
        QVERIFY(
            lruCache.payloadBytes()
            <= lruCache.payloadCapacityBytes());
        lruCache.resetStats();
        QCOMPARE(
            DocumentSerializer::toJson(
                *lruPrepared,
            ...
            originalJson);
        QCOMPARE(
            lruCache.stats().binaryMaskCompressions,
            2ULL);
        QVERIFY(
            lruCache.payloadBytes()
            <= lruCache.payloadCapacityBytes());
        ...
        QCOMPARE(
            cappedCache.payloadCapacityBytes(),
            DocumentSerializer::SerializationCache::
                maximumPayloadBytes);
    }
    ...
    void cachesFourKMaskAcrossAccumulatedStrokes()
    {
        constexpr int edge = 4096;
        Document source =
            Document::createDefault(QSize(edge, edge));
        PixelSelectionOp operation;
        operation.canvasSize = source.size;
        operation.sourceBounds =
            QRect(QPoint(), source.size);
        operation.packedMask = QByteArray(
            static_cast<qsizetype>(edge) * edge / 8,
            '\0');
        ...
        selectionStroke.mode = StrokeMode::PixelSelection;
        selectionStroke.points.clear();
        selectionStroke.pixelSelectionOp =
            std::move(operation);
        source.layers.first().strokes.append(
        ...
        QCOMPARE(
            cache.stats().binaryMaskCompressions,
            1ULL);
        ...
        for (int index = 0;
            ...
            candidate.layers.first().strokes.append(
                std::move(stroke));
            cache.resetStats();
            auto next = DocumentSerializer::prepare(
                std::move(candidate),
                cache,
                &*current,
                DocumentLimits::maximumProjectBytes);
        ...
        const QByteArray json =
            DocumentSerializer::toJson(*current, cache);
        QVERIFY(!json.isEmpty());
        QCOMPARE(
            current->compactSize(),
            static_cast<qint64>(json.size()));
        qInfo().nospace()
            << "4K prepared-cache benchmark: "
            << appendedStrokeCount
            << " accumulated strokes in "
            << elapsedMilliseconds
            << " ms";
    }
