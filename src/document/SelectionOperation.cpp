...
SamplingMode samplingForTransformImpl(const QTransform &transform)
{
    const auto integralUnit = [](qreal value) {
        return qFuzzyIsNull(value)
            || qFuzzyCompare(std::abs(value), 1.0);
    };
    ...
    const bool orthogonalUnit =
        integralUnit(transform.m11())
        && integralUnit(transform.m12())
        && integralUnit(transform.m21())
        && integralUnit(transform.m22())
        && qFuzzyIsNull(
        ...
        && qFuzzyCompare(
            transform.m21() * transform.m21()
                ...
                + 1.0,
            2.0);
    return orthogonalUnit
            && integralTranslation(transform.dx())
...
QRect mappedTargetBounds(
    const QRect &sourceBounds,
    const QSize &targetSize,
    const QTransform &transform)
{
    const QRectF mapped = transform.mapRect(QRectF(sourceBounds));
    if (!std::isfinite(mapped.left())
        || !std::isfinite(mapped.top())
        || !std::isfinite(mapped.right())
        || !std::isfinite(mapped.bottom())) {
        return {};
    }
    const qreal width = targetSize.width();
    const qreal height = targetSize.height();
    const qreal leftValue = std::clamp(
        mapped.left() - transformedBoundsMargin,
        0.0,
        width);
    const qreal topValue = std::clamp(
        mapped.top() - transformedBoundsMargin,
        0.0,
        height);
    const qreal rightValue = std::clamp(
        mapped.right() + transformedBoundsMargin,
        0.0,
        width);
    const qreal bottomValue = std::clamp(
        mapped.bottom() + transformedBoundsMargin,
        0.0,
        height);
...
bool integralOrthogonalTransform(
    const QTransform &transform)
{
    return finiteTransform(transform)
        && transform.isAffine()
        && transform.isInvertible()
        && samplingForTransformImpl(transform)
            == SamplingMode::Nearest;
}
    ...
    const QRect &sourceBounds)
{
    ...
    const QPoint offset =
        region.bounds.topLeft() - sourceBounds.topLeft();
...
RenderedMaskRegion renderBinarySource(
    QImage source,
    const QRect &sourceBounds,
    ...
    QImage::Format targetFormat)
{
    ...
    rendered.renderPeakBytes =
        rendered.sourceBytes + rendered.targetBytes;
    ...
    bool usedFullTargetFallback)
{
    ...
    stats->peakLiveImageBytes = std::max(
        rendered.renderPeakBytes,
        rendered.targetBytes + resultBytes);
}
    ...
    const QSize &targetSize)
{
    ...
    for (int y = 0; y < rendered.image.height(); ++y) {
        uchar *output =
            support.scanLine(y + rendered.bounds.y())
            + rendered.bounds.x();
        if (rendered.image.format() == QImage::Format_Alpha8
            ...
            const auto *input = reinterpret_cast<const QRgb *>(
                rendered.image.constScanLine(y));
            for (int x = 0; x < rendered.image.width(); ++x) {
    ...
    const QSize &targetSize)
{
    ...
    packed.bounds = localBounds.translated(
        rendered.bounds.topLeft());
    packed.packedMask = QByteArray(
        stride * localBounds.height(),
        '\0');
    for (int y = 0; y < localBounds.height(); ++y) {
        const uchar *input = rendered.image.constScanLine(
            localBounds.y() + y);
        auto *output = reinterpret_cast<uchar *>(
            packed.packedMask.data()
            + static_cast<qsizetype>(y) * stride);
    ...
    const PackedMaskRegion &region)
{
    if (region.canvasSize.width()
        ...
        || !QRect(QPoint(), region.canvasSize)
    ...
    if (remainder != 0) {
        ...
        for (int y = 0; y < region.bounds.height(); ++y) {
            const uchar last = static_cast<uchar>(
                region.packedMask.at(
                    static_cast<qsizetype>(y) * stride
                    + stride - 1));
    ...
    int documentY)
{
    ...
    const uchar byte = static_cast<uchar>(
        region.packedMask.at(
            static_cast<qsizetype>(localY) * stride
            + localX / 8));
    return (byte
            & static_cast<uchar>(0x80U >> (localX % 8)))
        != 0;
}
    ...
    SelectionTransformMemoryStats *memoryStats)
{
    ...
    if (!isValidPackedMaskRegion(region)
        || !validTransformTargetSize(targetSize)
    ...
    const bool useMappedTarget =
        integralOrthogonalTransform(transform);
    const QRect sourceBounds = useMappedTarget
        ? region.bounds
        : region.bounds.adjusted(-1, -1, 1, 1).intersected(
              fullImageBounds(region.canvasSize));
    const QRect targetBounds = useMappedTarget
        ? mappedTargetBounds(
            sourceBounds,
            targetSize,
            transform)
    ...
    const RenderedMaskRegion rendered = renderBinarySource(
        std::move(source),
        sourceBounds,
        targetBounds,
        transform,
    ...
    recordMemoryStats(
        ...
        rendered,
        packed
            ? static_cast<quint64>(packed->packedMask.size())
            : 0,
        !useMappedTarget);
    return packed;
}
...
bool isValidPixelSelectionOp(const PixelSelectionOp &operation)
{
    ...
    const auto safeCoefficient =
        [maximumCoordinate](qreal value) {
            return std::abs(value) <= maximumCoordinate;
        };
    const QRectF mappedBounds =
        operation.transform.mapRect(
            QRectF(operation.sourceBounds));
    if (operation.canvasSize.width()
            < DocumentLimits::minimumCanvasEdge
    ...
    const qsizetype stride =
        packedStride(operation.sourceBounds.width());
    if (stride <= 0
        || stride
            > std::numeric_limits<qsizetype>::max()
    ...
    if (remainder != 0) {
        const uchar unusedMask =
            static_cast<uchar>((1U << (8 - remainder)) - 1U);
        for (int y = 0;
             y < operation.sourceBounds.height();
             ++y) {
            const uchar last = static_cast<uchar>(
                operation.packedMask.at(
                    static_cast<qsizetype>(y) * stride
                    + stride - 1));
            if ((last & unusedMask) != 0) {
                return false;
    ...
    SelectionTransformMemoryStats *memoryStats)
{
    ...
    const bool oneChannelNearest =
        sampling == SamplingMode::Nearest
        && integralOrthogonalTransform(transform);
    const bool useFullTargetFallback =
        sampling == SamplingMode::Nearest
        && !oneChannelNearest;
    const QRect targetBounds = useFullTargetFallback
        ? fullImageBounds(targetSize)
    ...
    const QImage::Format sourceFormat = oneChannelNearest
        ? QImage::Format_Alpha8
        : QImage::Format_ARGB32_Premultiplied;
    const QImage::Format targetFormat = useFullTargetFallback
        ? QImage::Format_ARGB32_Premultiplied
        : QImage::Format_Alpha8;
    QImage source = binarySourceFromMask(
        selectionMask,
        bounds,
        sourceFormat);
    if (source.isNull()) {
    ...
    QImage support = supportFromRenderedRegion(
        rendered,
        targetSize);
    if (support.isNull()) {
        return {};
...
bool pixelSelectionContains(
    ...
    int documentY)
{
    if (!operation.sourceBounds.contains(
            documentX,
            documentY)) {
        return false;
    }
...
bool isValidReframeOp(const ReframeOp &operation)
{
    ...
    if (!validSize(operation.sourceSize)
        || !validSize(operation.targetSize)
