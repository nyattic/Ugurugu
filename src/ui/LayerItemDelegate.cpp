...
const QPixmap &eyeOpenPixmap()
{
    static const QPixmap pixmap =
        Icons::pixmap(IconGlyph::EyeOpen, eyeSize, Theme::textMuted(), 0.0, 2.0);
    return pixmap;
}
...
QRect LayerItemDelegate::thumbnailRect(const QRect &rowRect) const
{
    ...
    return QRect(
        rowRect.left() + rowPadding + accentBarWidth + 8,
        y,
        thumbnailWidth,
        thumbnailHeight);
}
...
QRect LayerItemDelegate::eyeRect(const QRect &rowRect) const
{
    ...
    return QRect(
        rowRect.right() - rowPadding - eyeSize,
        y,
        eyeSize,
        eyeSize);
}
    ...
    const QModelIndex &index) const
{
    ...
    if (selected) {
        const QRectF card =
            QRectF(option.rect).adjusted(4.0, 2.0, -4.0, -2.0);
    ...
    if (!thumbnail.isNull()) {
        const QSizeF drawnSize =
            thumbnail.deviceIndependentSize().scaled(
                thumbRect.size(),
                Qt::KeepAspectRatio);
    ...
    painter->drawRoundedRect(
        QRectF(thumbRect).adjusted(0.5, 0.5, -0.5, -0.5),
        4.0,
        4.0);

    painter->setPen(
        visible ? Theme::textPrimary() : Theme::textMuted());
...
void LayerItemDelegate::updateEditorGeometry(
    QWidget *editor,
    const QStyleOptionViewItem &option,
    const QModelIndex &index) const
{
