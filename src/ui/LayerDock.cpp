...
constexpr QSize thumbnailRenderSize(96, 64);
...
QPixmap renderLayerThumbnail(const Document &document, const Layer &layer)
{
...
void LayerDock::buildContent()
{
    ...
    m_duplicateButton = makeLayerButton(
        content,
        IconGlyph::Duplicate,
    ...
    m_deleteButton = makeLayerButton(
        content,
        IconGlyph::Remove,
    ...
    m_moveUpButton = makeLayerButton(
        content,
        IconGlyph::MoveUp,
    ...
    m_moveDownButton = makeLayerButton(
        content,
        IconGlyph::MoveDown,
...
void LayerDock::connectControls()
{
    connect(
        ...
        [this](QListWidgetItem *current) {
            ...
            const QUuid id =
                current->data(LayerItemRoles::LayerId).toUuid();
            if (!id.isNull()) {
    ...
    connect(
        ...
        [this](const QModelIndex &index) {
            ...
            const QUuid id =
                index.data(LayerItemRoles::LayerId).toUuid();
            const bool visible =
                index.data(LayerItemRoles::Visible).toBool();
            if (!id.isNull()) {
    ...
    connect(m_duplicateButton, &QToolButton::clicked, this, [this]() {
    ...
    connect(m_moveUpButton, &QToolButton::clicked, this, [this]() {
        const QUuid id = selectedLayerId();
        if (m_controller && !id.isNull()) {
            m_controller->moveLayer(id, 1);
        }
    });
    connect(m_moveDownButton, &QToolButton::clicked, this, [this]() {
        const QUuid id = selectedLayerId();
        if (m_controller && !id.isNull()) {
            m_controller->moveLayer(id, -1);
        }
...
void LayerDock::rebuild()
{
    ...
    if (m_controller) {
        ...
        for (int index = document.layers.size() - 1; index >= 0; --index) {
            ...
            item->setData(
                LayerItemRoles::LayerId,
                QVariant::fromValue(layer.id));
            item->setData(LayerItemRoles::Visible, layer.visible);
            item->setData(
                LayerItemRoles::Thumbnail,
...
void LayerDock::handleReorder(int sourceRow, int insertRow)
{
    if (!m_controller
        || sourceRow < 0
        || sourceRow >= m_layerList->count()) {
        return;
    }
    const QUuid id = m_layerList->item(sourceRow)
        ->data(LayerItemRoles::LayerId)
        .toUuid();
    const int finalRow =
        insertRow > sourceRow ? insertRow - 1 : insertRow;
...
QUuid LayerDock::selectedLayerId() const
{
    ...
    return item
        ? item->data(LayerItemRoles::LayerId).toUuid()
        : QUuid();
}
