...
Stroke previewStroke(const BrushPreset &preset)
{
    ...
    for (int index = 0; index < sampleCount; ++index) {
        const qreal t =
            static_cast<qreal>(index) / (sampleCount - 1);
        StrokePoint point;
        point.position = QPointF(
            (10.0 + t * (previewWidth - 20.0)) * previewScale,
            (previewHeight * 0.5
             - std::sin(t * std::numbers::pi * 1.5) * 5.0)
                * previewScale);
...
BrushPresetButton::BrushPresetButton(
...
void BrushPresetButton::paintEvent(QPaintEvent *event)
{
    ...
    const QRectF frame =
        QRectF(rect()).adjusted(0.75, 0.75, -0.75, -0.75);
    QPainterPath card;
    card.addRoundedRect(frame, 8.0, 8.0);
    QColor background = isChecked()
    ...
    painter.setPen(
        isChecked()
            ? QPen(Theme::accent(), 1.5)
            : QPen(Theme::border(), 1.0));
    painter.drawPath(card);

    painter.drawImage(
        QPointF(
            (width() - previewWidth) * 0.5,
            5.0),
        frameImage(m_frame));
...
const QImage &BrushPresetButton::frameImage(int frame)
{
    ...
    Document document;
    document.size = QSize(
        qRound(previewWidth * previewScale),
