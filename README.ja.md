...
Windowsビルドはコード署名されていないため、SmartScreenが警告を表示する
ことがあります。**詳細情報 → 実行**を選んで進めてください。アプリは
必ず公式Releasesページからのみダウンロードしてください。
...
macOSビルドはDeveloper IDで署名され、Appleの公証を受けているため、
追加の操作なしにそのまま開けます。
...
- 完全な透明にもできるキャンバスの背景色。背景のないステッカー風の
  PNGやGIFを作れます
- 進捗表示とキャンセルに対応したループGIFとPNG・JPG静止画の書き出し、
  `.wagle`プロジェクトファイル。GIFはサイズと透明部分の扱いを選べ、
  書き出す前に必要なメモリを確認できます
