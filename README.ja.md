...
- フレームごとに独立して描き直される、完全にループ可能な
  ボイリングラインウォブル
- 選択範囲の下に表示されるアクションバーで、移動・拡大縮小・回転・
  左右／上下反転を組み合わせてプレビューしてから適用または
  キャンセルでき、複製・削除・選択解除・取り消しにも対応した
  なげなわ選択と自動選択
- 選択範囲を認識するブラシ・消しゴム・塗りつぶし編集
- 絵を拡大縮小せず、絶対・相対サイズ、3×3基準点、オフセットを
  指定して切り抜き・拡張できるキャンバス領域サイズ変更
- 正確なピクセルサイズまたは倍率を指定し、必要に応じて縦横比を
  維持しながら絵とブラシサイズを一緒に拡大縮小できる画像サイズ変更
- 1～1600%の対数ズームスライダー、実ピクセルの100%表示、
  ウィンドウに合わせる表示
- サムネイル・表示切り替え・不透明度・ドラッグ並べ替えに対応し、
  最後の1枚も削除してレイヤー0枚の空キャンバスにできるレイヤー
...
- 異常終了後の未保存作業の自動復元
- アプリを再起動しても保持される、最後に使用したツール・色・
  ブラシプリセット・プリセットごとのブラシサイズ・消しゴムサイズ・
  粗さ・アンチエイリアス
