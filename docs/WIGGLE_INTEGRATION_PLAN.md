...
새로운 시간·공간 흔들림 설정은 문서 전역으로 둔다. 기존
`BrushSettings::wobbleScale`은 이전 문서의 렌더 호환을 위해 유지하지만
UI에 노출하지 않는다. 새 스트로크는 프리셋 기본값 `1.0`을 사용한다.
...
지우개 흔들림은 별도 배율 옵션을 두지 않고 문서 전역 설정을 따른다.
기존 지우개 스트로크의 `wobbleScale` 값은 렌더 호환을 위해 바꾸지 않는다.
