...
모든 선이 꿈틀거리는 와글와글 드로잉 앱입니다. 한 번만 그리면
스케치가 살아 있는 듯 와글거리는 애니메이션으로 깨어나고,
...
- 필압을 지원하는 브러시·지우개와 펜, 마커, 에어브러시, 스프레이
  프리셋 17종
- 손이 떨려도 선이 매끄럽게 그어지는 손떨림 보정 (브러시와 지우개에
  각각 0~100%로 조절)
- 프레임마다 다시 그려져 반복 재생되는 와글거림 (끄고 일반 그림툴처럼
  쓸 수도 있습니다)
- 선마다 가장자리를 또렷한 픽셀로 할지 부드럽게 할지 조절
- 이미 그린 선을 다시 그리지 않고 색상·굵기만 나중에 바꾸기
- 긴 선과 고해상도 캔버스에서도 끊김을 줄인 그리기·이동·확대/축소
- 이동, 크기 변경, 회전, 반전을 미리 보고 적용하는 올가미 선택과
  자동 선택, 선택 삭제까지 그림과 함께 실행 취소·다시 실행
- 그림을 유지한 채 자르고 넓히는 캔버스 크기 변경과, 그림째 확대·축소
  하는 이미지 크기 변경
- 섬네일, 표시 여부, 불투명도, 드래그 순서 변경을 지원하는 레이어와,
  여러 레이어를 폴더처럼 묶는 레이어 그룹
- 일반·곱하기·스크린·오버레이 합성 모드와, 바로 아래 레이어에 그려진
  부분에만 얹히는 클리핑
- 와글거림 강도, 프레임 수, FPS를 조절하는 타임라인과 실시간 미리보기
- 1~1600% 줌, 실제 픽셀 100% 보기, 창 맞춤
- 투명하게도 지정할 수 있는 캔버스 배경색. 스티커처럼 배경 없는 PNG와
  GIF를 만들 수 있습니다
- 진행률과 취소를 지원하는 반복 GIF·Animated WebP·PNG·JPG 내보내기와
  `.wagle` 프로젝트 파일. 애니메이션은 크기와 투명도를 고르고,
  내보내기 전에 필요한 메모리를 미리 확인할 수 있습니다
- WiggleWiggleTool v10 `.wawa` 프로젝트 가져오기와 WWP `.wwpreset`
  설정 프리셋 가져오기·내보내기
- 원하는 테마 색상과 `F1` 앱 내 도움말
- 비정상 종료 후 자동 복구, 마지막 도구·색상·브러시 설정 유지
...
- **일반:** 인터페이스 언어와 테마 색상을 선택합니다. 언어를 변경한
  뒤에는 앱을 다시 시작하세요.
...
WagleWaglePaint는 실행할 때 업데이트를 확인합니다. 마지막으로 확인한 지
여섯 시간이 지나지 않았다면 건너뛰므로, 하루에 여러 번 켜도 자주
확인하지는 않습니다. **도움말 → 업데이트 확인**으로 언제든 직접 확인할
수도 있습니다. 업데이트는
