...
- 프레임마다 다시 그려져 반복 재생되는 와글거림 (끄고 일반 그림툴처럼
  쓸 수도 있습니다)
- 선마다 가장자리를 또렷한 픽셀로 할지 부드럽게 할지, 거칠기는 얼마나
  줄지 조절
- 이미 그린 선을 다시 그리지 않고 색상·굵기·거칠기만 나중에 바꾸기
...
- 투명하게도 지정할 수 있는 캔버스 배경색. 스티커처럼 배경 없는 PNG와
  GIF를 만들 수 있습니다
- 진행률과 취소를 지원하는 반복 GIF·PNG·JPG 내보내기와 `.wagle`
  프로젝트 파일. GIF는 크기와 투명도를 고르고, 내보내기 전에 필요한
  메모리를 미리 확인할 수 있습니다
